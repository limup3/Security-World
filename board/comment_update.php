...
if(empty($w) || $w === 'w') { //$w 변수가 비어있거나 w인 경우
  $msg = '작성';
  $sql = 'insert into comment_free values(null, ' .$bNo . ', ' . $coNo . ', "' . $coContent . '", "' . $coId . '","' . $coPassword . '"))';
  ...
  $sql = 'select count(*) as cnt from comment_free where co_password="' . $coPassword . '" and co_no = ' . $coNo;
  $result = $db->query($sql);
...
<?php
  ...
  $sql = 'update comment_free set co_content = "' . $coContent . '" where co_password="' . $coPassword . '" and co_no = ' . $coNo;
...
<?php
  ...
  $sql = 'delete from comment_free where co_password="' . $coPassword . '" and co_no = ' . $coNo;
